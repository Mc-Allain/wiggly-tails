...
class ViewCustomerModal extends Component {
  ...
  componentDidMount() {
    ...
    record.homeAddress.street = customer.street.substring(
      0,
      customer.street.length - 4
    );
    record.homeAddress.subdivision = customer.subdivision;
    record.homeAddress.barangay = customer.barangay.substring(
      6,
      customer.barangay.length
    );
    record.homeAddress.municipality = customer.municipality.substring(
      0,
      customer.municipality.length - 5
    );
  ...
  onSubmit = () => {
    ...
    if (this.validForm({ errors })) {
      ...
      axios
        .post(
          "https://princemc.heliohost.us/veterinaryClinic/updateCustomerEmployee.php",
          record
        )
        .then(() => {
          onRefresh();
  ...
  onReset = () => {
    ...
    homeAddress.street = customer.street.substring(
      0,
      customer.street.length - 4
    );
    homeAddress.subdivision = customer.subdivision;
    homeAddress.barangay = customer.barangay.substring(
      6,
      customer.barangay.length
    );
    homeAddress.municipality = customer.municipality.substring(
      0,
      customer.municipality.length - 5
    );
    ...
    return (
      <React.Fragment>
        <div
          ...
          aria-labelledby={"viewCustomerrModalTitle-" + customer.id}
          aria-hidden="true"
        >
          <div
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5
                  ...
                  id={"viewCustomerModalTitle-" + customer.id}
                >
                  Customer Info
                </h5>
                {(!submitted && this.props.connected) || deleted ? (
                  <button
                    id={"btnClose-" + customer.id}
                      ...
                      {submitted || deleting || deleted ? (
                        ...
                        <React.Fragment>
                          <input
                            className={
                              "zi-10 " +
                              this.inputFieldClasses(errors.userPassword)
                            }
                            type={passwordState.inputType}
                            name="userPassword"
                            value={record.userPassword}
  ...
  onDelete = () => {
    ...
    axios
      .post(
        "https://princemc.heliohost.us/veterinaryClinic/deleteCustomer.php",
        record
      )
