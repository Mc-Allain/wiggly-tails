    ...
    switch (name) {
      ...
      case "adminPassword":
        errors.adminPassword =
          value.length === 0
            ? "Please input your Admin Password"
            : value.length > 24
  ...
  onSubmit = () => {
    ...
    if (this.validForm({ errors })) {
      ...
      const result = records.filter(
        (row) =>
          row.id === record.adminId && row.empPassword === record.adminPassword
      );
      ...
      if (result.length > 0) {
        ...
        history.replace("/wiggly-tails/admin/manage-employees", {
          ...
          adminId: result[0].id,
          adminName: result[0].empLastName + ", " + result[0].empFirstName,
          empType: result[0].empType,
        });
      } else {
        submitError = true;
        loginError = true;
    ...
    if (this.state.loginError) {
      return (
        <div className="alert alert-danger d-flex align-items-center mb-2 py-0">
          ...
          <span className="lh-0 my-1">
            Incorrect Admin ID or Admin Password
          </span>
        </div>
      );
    }
  ...
  renderModal = () => {
    ...
    return (
      <div
        ...
        id="manageEmployeesModal"
        tabIndex="-1"
        role="dialog"
        data-backdrop="static"
        aria-labelledby="manageEmployeesModalTitle"
        aria-hidden="true"
      >
        <div className="modal-dialog modal-dialog-centered" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="manageEmployeesModalTitle">
                Manage Employees
              </h5>
              <button
                id="btnClose"
